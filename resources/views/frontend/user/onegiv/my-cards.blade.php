...
@endif
    ...
    <div class="col-lg-4 col-md-6 mb-4">
        ...
        <div class="p-4 rounded-4 text-white mb-3"
            ...
            <div class="d-flex justify-content-between">
                ...
                <div class="text-end">
                    ...
                    {{-- User ID Status --}}
                    @if($card->user_id)
                        <span class="badge bg-success mt-1">User: {{ $card->user_id }}</span>
                    @else
                        <span class="badge bg-warning text-dark mt-1">No User</span>
                    @endif
        ...
        {{-- Assign Button — শুধু user_id না থাকলে দেখাবে --}}
        @if(!$card->user_id)
            <a href="{{ route('onegiv.assigncard', $card->id) }}"
               class="btn btn-sm w-100 text-white"
               style="background:#e67e22;"
               onclick="return confirm('Assign YOUR user ID ({{ Auth::id() }}) to this card?')">
                👤 Assign My User ID
            </a>
        @else
            <div class="btn btn-sm w-100 btn-success disabled">
                ✅ Assigned to User {{ $card->user_id }}
            </div>
        @endif
