        ...
        <div class="row ">
            ...
            <div class="col-lg-6"> 
                <div class="calculatior mt-3" style="min-height: 0px">
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="fw-bold fs-23 txt-secondary border-bottom pb-2">Add to card</div> <br>
                            ...
                            <div class="data-container">
                                ...
                                <table  class="table inner table-theme mt-0" id="">
                                    <tbody id="cardinner" class="right">
                                        ...
                                        <tr class="item-row" style="position:realative;"> 
                                            <td width="40px"> <div style="color: white; user-select:none; padding: 2px; background: red; width: 45px; display: flex; align-items: center; margin-right:5px; justify-content: center; border-radius: 4px; left: 4px; top: 8px;" data-entity-id="{{ $item['voucherID'] }}" data-cart-index="{{ $loop->index }}" class="remove-from-cart">X</div>
                                            </td>
                                            <td width="250px"> 
                                                <div class="title">
                                                    
                                                    <input type="hidden" value="{{ $item['voucherID'] }}" name="v_ids[]">
                                                    @if ($item['v_type'] == 'Prepaid') 
                                                    <input type="hidden" class="total" id="sub{{ $item['voucherID'] }}" value="{{ $item['v_amount'] }}">
                                                    @else 
                                                    <input type="hidden" class="total" value="">
                                                    @endif
                                                    @if ($item['single_amount'] == "0" ) <span class="bottom-data">Blank Cheque</span> @else <span class="bottom-data">£{{ $item['single_amount'] }}   </span> @endif

                                                    <div class="badge @if ($item['v_type'] == "Prepaid") prepaid @else postpaid @endif">
                                                        {{ $item['v_type'] }}
                                                    </div>
                                                </div>
                                                <span class="bottom-data"> {{$item['v_note']}} @if ($item['v_type'] == 'Prepaid') = £{{ $item['v_amount']  }} @endif</span>
                                            </td>

                                            <td width="80px"> <input style="min-width: 50px;" type="text" name="qty[]" class="form-control qty" onkeypress="return /[0-9]/i.test(event.key)" value="{{$item['quantity']}}"  v_amount="{{ $item['v_amount'] }}" v_type="{{ $item['v_type'] }}" data-type="{{ $item['v_type'] }}" vid="{{$item['voucherID']}}"  id="cartValue{{$item['voucherID']}}"> </td>
                                            
                                            
                                            <td width="150px">  <input style="min-width: 50px;" type="number" name="total[]" class="form-control vtotal" id="vtotal{{$item['voucherID']}}" @if ($item['v_type'] == 'Prepaid') value="{{ $item['v_amount'] }}" @else value="" @endif readonly> </td>
